...
def get_cam_detections(observation):
    ...
    if observation.left is not None:
        obs_dir_list[AgentPercepts.LEFT] = observation.left

    if observation.forwardleft is not None:
        obs_dir_list[AgentPercepts.FRONT_LEFT] = observation.forwardleft

    if observation.forward is not None:
        obs_dir_list[AgentPercepts.TOP] = observation.forward

    if observation.forwardright is not None:
        obs_dir_list[AgentPercepts.FRONT_RIGHT] = observation.forwardright

    if observation.right is not None:
        obs_dir_list[AgentPercepts.RIGHT] = observation.right
